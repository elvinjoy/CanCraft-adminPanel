...
const ManagerLogin = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        setLoading(true);
        try {
            const response = await axios.post(`${BASE_URL}/managers/login`, formData);
            if (response.status === 200) {
                const { manager, token } = response.data;
    ...
    return (
        <div className="container d-flex justify-content-center align-items-center vh-100">
            <div className="border p-5 position-relative" style={{ maxWidth: '500px', width: '100%' }}>
